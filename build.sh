...
if echo "$@" | grep -v "force" 2>/dev/null >/dev/null; then
  echo "check if image was already built and pushed - skip check on release version"
  echo "$@" | grep -v "release" && docker pull "$IMG:$TAG" 2>/dev/null >/dev/null && echo "image already built" && exit 1
fi
